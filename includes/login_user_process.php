...
session_start();
	...
	if (isset($_POST['submit-btn'])) {
		...
		if (empty($email) || empty($password)) {
			header("Location: ../login_user.php?login=empty");
			exit();

		}else {

			//Check if there is email that has been registered
			$sql = "SELECT * FROM jj_inv_account WHERE email='$email'";
			$result = mysqli_query($conn, $sql);
			$resultCheck = mysqli_num_rows($result);

			if ($resultCheck < 1) {
				header("Location: ../login_user.php?login=emailnotfound");
				exit();
			}else {
				if ($row = mysqli_fetch_assoc($result)) {
					$hashPassword = password_verify($password, $row['password']);
					if ($hashPassword == false) {
						header("Location: ../login_user.php?login=passwordincorrect");
						exit();
